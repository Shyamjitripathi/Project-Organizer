...
function Dashboard(props) {
  ...
  useEffect(() => {
    async function fetchProjectData() {
      if (loggedInUser && loggedInUser._id) {
        const dataResult = await fetch(
          `/projectboard.up.railway.app/projects/${loggedInUser._id}/page/${page}`,
          {
            method: "GET",
          }
        );
  ...
  useEffect(() => {
    async function getRecentProjects() {
      if (loggedInUser && loggedInUser._id) {
        ...
        const rawData = await fetch(`/projectboard.up.railway.app/projects/${loggedInUser._id}/profile`);
